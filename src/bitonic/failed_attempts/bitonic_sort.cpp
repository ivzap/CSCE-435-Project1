 ...
 * increasing 0 for decreasing
 */
void compare(int rank_to_send_to, int arr[], int num_elements, int low, int count,
             bool direction);
 ...
 * Start Bitonic Sort
 ******************************************************************************/
bitonic_sort_p(local, elements_per_proc, num_procs, rank);
/******************************************************************************
 * End Bitonic Sort
 ******************************************************************************/
/******************************************************************************
...
bool sorted;
if (rank == 0) {
	sorted = is_sorted_p(local, num_elements, rank, num_tasks);
}
...
if (rank == 0 && sorted) {
	...
	std::cout << "Failed sorting" << std::endl;
	return 1;
}
/******************************************************************************
...
void compare(int rank_to_send_to, int arr[], int num_elements, int low, int count,
  ...
  std::cout << "sendrecv to process " << rank_to_send_to << std::endl;
  MPI_Sendrecv(arr, num_elements, MPI_INT, rank_to_send_to, 0, recv, num_elements, MPI_INT, rank_to_send_to, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  bitonic_merge(arr, low, count, direction);
}

void bitonic_sort_p(int arr[], int num_elements, int total_processes, int rank) {
  int dimension = static_cast<int>(log2(total_processes));
  std::cout << "total_processes " << total_processes << std::endl;
  std::cout << "dimension " << dimension << std::endl;
  std::sort(arr, arr + num_elements);

    for (up=2;up<=local_amount*num_procs;up=2*up)
    {
        for (j=up>>1;j>0;j=j>>1)
        {
            for (i=0;i<local_amount*num_procs;i++)
            {
                int partner=i^j;                
                if ((partner)>i)
                {   
                    compare(partner, arr , num_elements,i&up);
                }
